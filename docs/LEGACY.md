...
[Click here](https://pypi.org/project/adestis-netbox-applications/)
...
$ cd adestis-netbox-applications
$ docker build -f Dev-Dockerfile -t adestis-netbox-applications-plugin .
```
...
Mount the migrations folder, start the infrastructure services and run `docker-compose run netbox sh -c "python manage.py makemigrations adestis_netbox_applications"`. After that unmount the migrations folder and rebuild the docker container
